...
const PricingPage = () => {
  const createUser = useMutation(api.users.createUser); // ✅ move inside the component

  useEffect(() => {
    const create = async () => {
      try {
        await createUser({ name: "John Doe", email: "john@example.com", role: "guest", password: "123456",});
        console.log("User created successfully on PricingPage");
      } catch (err) {
        console.error("User creation failed on PricingPage:", err);
      }
    };

    create(); // ✅ call inside useEffect
  }, [createUser]);

  return (
    <div>
      <div
        className="bg-cover md:bg-contain bg-no-repeat bg-top h-[60vh] md:h-[75vh] lg:h-[85vh]"
        ...
        <div className='flex items-center justify-center gap-10 md:gap-8 lg:gap-14 px-4 w-[100%] flex-wrap'>
          {[1, 2, 3, 4].map((_, idx) => (
            <Link
              to="/payment"
              key={idx}
              className="cursor-pointer w-[190px] rounded-2xl bg-[#f5f5f5] relative p-7 border-2 border-[#c3c6ce] transition ease-out duration-500 overflow-visible group hover:border-[#008bf8] hover:shadow-[0_4px_18px_0_rgba(0,0,0,0.25)]"
            >
